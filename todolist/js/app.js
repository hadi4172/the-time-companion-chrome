(function (context) {
    ...
    App.fn.showTotalTasksLeft = function (items) {
        ...
        var text = [len, " " + chrome.i18n.getMessage("todolist_2"), $.pluralization(len),  " " + chrome.i18n.getMessage("todolist_2_2")].join("");
    ...
    window.addEventListener("DOMContentLoaded", function () {
        ...
        sortableInstance = Sortable.create(document.querySelector("#js-list"), {
            ...
            store: {
                ...
                set: function (sortable) {
                    ...
                    let elements = Array.from(sortable.el.childNodes)
                        .filter(e => e.className !== "completed")
                        .map(e => e.outerText);
                    // console.log(`elements`, elements);
                    chrome.storage.local.set({ todo: elements });
                    localStorage.setItem(sortable.options.group.name, order.join("|"));
                },
            },
        });

        document.querySelector("#js-insert").placeholder = chrome.i18n.getMessage("todolist_1");
        document.querySelector('a[href="#/all"]').innerHTML = chrome.i18n.getMessage("todolist_3");
        document.querySelector('a[href="#/active"]').innerHTML = chrome.i18n.getMessage("todolist_4");
        document.querySelector('a[href="#/completed"]').innerHTML = chrome.i18n.getMessage("todolist_5");
        document.querySelector("#js-clear-completed").innerHTML = chrome.i18n.getMessage("todolist_6");
